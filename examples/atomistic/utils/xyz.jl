...
"""
function PotentialLearning.load_data(file::String, energy_units::Unitful.FreeUnits,
                   distance_units::Unitful.FreeUnits; nmax = Inf, T = Float64)
    ext = []
...
"""
function PotentialLearning.load_data(file, extxyz::ExtXYZ; nmax = Inf, T = Float64)
    configs = Configuration[]
    open(file, "r") do io
        count = 1
        while !eof(io) && count <= nmax
